...
- Anytype MCP server configured in `opencode.json` (see `setup/anytype-mcp-config.json`)
- API key created in Anytype Settings > API Keys — set as `<YOUR_API_KEY>` in `opencode.json`
...
At the beginning of each new session, automatically run:
1. `bash .opencode/scripts/session-start.sh` — reads `.space.md` and checks Anytype connectivity
2. Search for `brain_note` named "North Star" — ground suggestions in current goals
3. Search for `work_note` objects with `status=active` — see current projects
4. Search for `brain_note` named "Memories" — index of memory topics

Run `/standup` for a full structured morning kickoff.

## Content Classification (Routing Hints)

When the user's message contains any of the following, act on the corresponding routing hint:

| Signal | Routing |
|--------|---------|
| `decided`, `decision`, `agreed to`, `we chose` | Create a `decision` type object |
| `incident`, `outage`, `pagerduty`, `p0/p1/sev1/sev2`, `postmortem` | Use `/incident-capture` or create `incident` object |
| `1:1`, `1-on-1`, `catch up with`, `sync with` | Create `one_on_one` object, update `person` object |
| `shipped`, `launched`, `completed`, `released`, `deployed`, `kudos`, `win` | Create `brag_entry` object with evidence relations |
| `architecture`, `system design`, `rfc`, `tech spec`, `adr` | Create `brain_note` or `decision` object |
| `told me`, `feedback from`, `spoke with`, `mentioned` | Update relevant `person` object |
| `project update`, `sprint`, `milestone`, `merged`, `rolled out` | Update active `work_note` object |

Always use proper `type_key` values, set required properties, and create relations. Follow the conventions below.
...
The `session.created` plugin hook runs `session-start.sh` automatically: checks connectivity and logs space/Anytype status. For a full morning kickoff, run `/standup`.
...
Specialized agents in `.opencode/agents/` for heavy operations.
...
Invoke with `@brag-spotter`, `@context-loader`, etc.

## Hooks (opencode plugin)

Implemented in `.opencode/plugins/anytype-mind.js`:

| Hook | When | What |
|------|------|------|
| session.created | On new session | Runs `session-start.sh` — logs space ID and Anytype connectivity |
| tool.execute.before | Before MCP writes | Validates object properties and relations |
| session.compacted | Before context compaction | Backs up session transcript |
| session.idle | End of session | Lightweight checklist reminder |

Note: Claude Code's `UserPromptSubmit` hook (message classification) is replaced by the Content Classification table in this file.
